...
module.exports = function runJsTask(userConfig) {
  ...
  const js = () => Promise.all([libraries(), ...fileTasks]);

  // if package.json changes, re-run all js tasks
  quench.maybeWatch(R.unnest([config.watch, findup.sync("package.json")]), js);
...
function createNpmPackagesGetter(files, librariesTask) {
  ...
  // it will start populated, so libraries() must be run manually initially.
  // this allows for the parent task to wait for libraries to finish
  ...
  return function getNpmPackages() {
    ...
    // if the list is different, re-run libraries task
