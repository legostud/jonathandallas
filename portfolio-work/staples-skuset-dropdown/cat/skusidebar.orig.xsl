								...
								<xsl:when test="./@priceincart='1'">
									...
									<xsl:element name="a">
										...
										<xsl:if test="($presentationType = 'D')">
											<select id="skuSelectControl" name="skuSetSelection">
												<xsl:attribute name="onChange">SWATCH.selectDropdown('selectDropdown');</xsl:attribute>
												<xsl:for-each select="../product">
													<xsl:if test="@type ='skuset'">
														<xsl:element name="option">
															<xsl:attribute name="id"><xsl:text>dropdown_</xsl:text><xsl:value-of select="@id" /></xsl:attribute>
															<xsl:attribute name="value"><xsl:value-of select="@id" /></xsl:attribute>
															<xsl:attribute name="rel">
																<xsl:value-of select="@snum" />_<xsl:value-of select="@id" />
															</xsl:attribute>
															<xsl:if test="$skuType ='skuset'">
																<xsl:attribute name="selected"><xsl:value-of select="$label_selected" /></xsl:attribute>
															</xsl:if>
