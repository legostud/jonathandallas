...
///////////////////////////////////////
/// iescene7.js merging starts here ///
///////////////////////////////////////
( function(window, document, $, undefined) {
    'use strict';
    var STAPLES = ( function(STAPLES) {

		STAPLES.Scene7 = ( function() {
			var S7ConfigClient,
				root,
				imageServer,
				myName = STAPLES.SKU.genInstance(),
				viewerType = "flyoutviewer",  //this declares the viewer type to load by the name of the loader file minus the .swf extension
				imageName,// = "Staples/s0404855_sc7";  //Image to pass to the viewer
				flashvars = {},
				viewerUrl = "";  //Build Viewer Path to Viewer swf
			
			function S7ConfigObject() {  
			    //Change the urls below to match Staples image server urls
			...
			S7ConfigObject.prototype.setFlashParam = function(inId) {
			    ...
			    divcontainer = "flash_setvariables_" + inId + "_" + this.divId;
			    this.divId ++;
			...
			function init() {
				S7ConfigClient = new S7ConfigObject();
				root = S7ConfigClient.isViewerRoot;
				imageServer = S7ConfigClient.isRoot;
				//Please note that viewer arguments not referenced here are not suitable for this viewer type and will not have any effect on viewer behavior
				//var viewerUrl = "";  //Build Viewer Path to Viewer swf
				viewerUrl += S7ConfigClient.isViewerRoot;
				viewerUrl += "flash/" + viewerType + ".swf?";
				viewerUrl += "&serverUrl=" + S7ConfigClient.isRoot;
				viewerUrl += "&instanceName=" + myName;
				viewerUrl += "&border=ffffff,0"; //Enables optional border around view. Hex RGB color value and thickness in pixels.
				viewerUrl += "&transparent=true"; //Enables transparent stage and swf-alpha requests. 
				if (pageId === 'skuskuset' || pageId === "collection"){ //if skuset/single sku page
				    viewerUrl += "&flyout=400,200,360,0"; //Specifies the pixel size and position of the flyout window  flyout = wid , hei [ , left , top]
				}else{ //quickview
				    viewerUrl += "&flyout=420,200,360,0";
				}
				viewerUrl += image_location //add image location set in the <head> of the sku/skuset page or in the quickview html.
				//viewerUrl += "&tips=Hover mouse cursor over this image to bring up the zoom view, Mouse out of the image to close the zoom view,black, 10"; //Text may be displayed at the bottom of the main view to provide a hint the user that hovering the cursor over the image will provide a magnified view. The text area is filled with the color specified with border=.  tips = normString, activeString [ , fontColor [ , fontSize ] ]
				viewerUrl += "&zoomFactor=3.0"; //Specifies the image magnification for the flyout view, relative to the main view.  Default value of 3.0 must be 1.0 or greater.
				viewerUrl += "&transition=fade"; //Specifies the type and duration of the effect applied when the flyout view is shown and hidden.  transition = effect , [ duration ] effect none | wipe | fade | dissolve duration transition duration in seconds (real)
